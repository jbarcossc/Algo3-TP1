...
Cuadrado numeroMagico::rellenarCasilla(int fila, int columna, int valor) {
    // Debe cumplir: fila y columna en rango y valor válido
    if(valor > 0 && valor <= this->n*this->n && fila >= 0 && fila < this->n && columna >= 0 && columna < this->n){
...
ostream&  numeroMagico::operator<< (std::ostream &out){
...
ostream &numeroMagico::operator=(numeroMagico &b) {
    this->n = b.size();
    this->mat = b.cuadrado();
}
